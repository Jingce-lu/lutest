...
Checkbox 新增的 prop： `label` 只会在组合使用时有效，结合 `model` 来使用，用法已在 Vue.js 文档中介绍了 [cn.vuejs.org/v2/guide/fo…](https://cn.vuejs.org/v2/guide/forms.html#复选框)。
...
参考本节的代码，实现一个单选组件 Radio 和 RadioGroup。
### 结语
你看到的简单组件，其实都不简单。
