...
![](https://user-gold-cdn.xitu.io/2018/11/27/16753fd7a86231b6?imageslim)
...
```
在 node.vue 中，通过 `findComponentUpward` 向上找到了 Tree 的实例，并调用它的 `emitEvent` 方法来触发自定义事件 `@on-toggle-expand`。之所以使用 f`indComponentUpward` 寻找组件，而不是用 `$parent`，是因为当前的 node.vue，它的父级不一定就是 tree.vue，因为它是递归组件，父级有可能还是自己。
...
以上就是 Tree 组件的所有内容，完整的代码见： [github.com/icarusion/v…](https://github.com/icarusion/vue-component-book/tree/master/src/components/tree)

用例：[github.com/icarusion/v…](https://github.com/icarusion/vue-component-book/blob/master/src/views/tree.vue)
